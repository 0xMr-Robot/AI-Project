...
def predict_home_automation(mood, person_condition, time_of_day, at_home, is_holiday):
    ...
    try:
        ...
        input_data = np.array([[
            mood_mapping[mood],
            condition_mapping[person_condition],
        ...
        for light, status in lights.items():
            ...
            if status == 1:
                recommendations.append(f"Turn ON {room} Light")
            else:
                recommendations.append(f"Turn OFF {room} Light")
        
        # Music recommendation
        if at_home == 1:
            recommendations.append(f"Suggested Music Type: {music_prediction}")
        else:
            recommendations.append("Music: NONE (Not at home)")
...
def validate_input(current_time, current_date, person_condition, at_home, 
    ...
    try:
        ...
        if not isinstance(light_level, (int, float)) or light_level < 0 or light_level > 1000:
            raise ValueError("Light level must be between 0 and 1000")
